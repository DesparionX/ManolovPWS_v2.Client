...
- **Cold-start resilience (Railway).** The backend sleeps after inactivity and cold-starts on the next request — the container can take several seconds to actually start accepting connections. Left alone, a request during that window either hangs (native `fetch` has no timeout of its own — it would wait on the OS's own TCP timeout, commonly minutes, with zero feedback and no recovery) or Railway's own edge proxy fails it early with a `502`/`503`/`504` before the app is up. `httpClient.ts`'s internal `fetchWithRetry` wraps every request (queries **and** mutations) in a bounded per-attempt timeout (`REQUEST_TIMEOUT_MS = 15_000`, via `AbortController`) plus up to `MAX_ATTEMPTS = 3` attempts, `RETRY_DELAY_MS = 2_000` apart, retrying only on: a timed-out/failed attempt (no response at all), or a real response whose status is `502`/`503`/`504` (a proxy-can't-reach-the-app status, not something the app itself produced — any other status means the app actually responded, so it's returned immediately, retry or not). If every attempt still fails, `apiFetch` throws a normal `ApiError` with a clear message ("Couldn't reach the server" / "The server is starting up") — never the raw `AbortError`/`TypeError`, which would silently bypass every error handler the same way the empty-body bug above did.
  - **Chosen over just raising a single attempt's timeout very high:** one long wait has no recovery path if that guess is still too short, while a few bounded attempts adapt to whatever the actual cold-start duration turns out to be. An already-warm backend is unaffected either way — the first attempt succeeds immediately, same as before this existed.
  - **Safe for mutations too, unlike `mutations: { retry: 0 }` below might suggest.** That default exists because blindly retrying *any* final failure risks duplicating a write whose response was merely lost after the app already processed it — a real risk for non-idempotent endpoints (`POST /Posts`, etc.). This retry is narrower and doesn't share that risk: it only ever fires when the app never received/processed the request in the first place (a timeout, a dropped connection, or a proxy-level 502/503/504), so there's nothing server-side to duplicate. The two mechanisms operate at different layers and don't conflict — `fetchWithRetry` resolves (or gives up) entirely before TanStack Query's own `retry` config ever sees a result.
  - **`refreshAccessToken()`'s own request goes through the same `fetchWithRetry`** (not a bare `fetch`) — the silent-refresh call hits the same backend and is equally susceptible to a cold container, most plausibly right after a page reload following a period of idle.
