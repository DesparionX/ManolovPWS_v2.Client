...
- **Renders the real HTML (`dangerouslySetInnerHTML`), not stripped-to-text** — same reversal as Home's `PostCard` (see `pages/HOME.md`'s Future Considerations): `line-clamp-2` only visually hides overflow, so it's exactly as safe to clamp actual rendered markup as it was to clamp plain text, and the preview no longer drops real formatting the Owner authored. `ProjectCard.tsx` renders `project.description` directly, same styling hooks as `PROJECT-DETAIL.md`'s rich-text block.
