...
- **Mobile-only: the toggle button shows an icon (`Pencil` for "Edit", `Eye` for "Preview") instead of text.** Desktop keeps the original text-only button unchanged (`EditToggleButton` in `ProfileMainTab.tsx` renders both the icon and the label always; each is hidden/shown per breakpoint via Tailwind, not conditionally rendered).
...
1. **Default view:** compact table-like list, one row per item, showing only the 1-2 key summary fields (not all data)
   - **Education, Experience, and Certificates display most-recent-first**, not in whatever order they happen to sit in the underlying array — `sortByDateDesc` (`shared/utils/sortByDateDesc.ts`, shared with the CV page's own identical need, see `pages/cv.md`) sorts a _display_ copy by `startDate` (Education/Experience) or `dateObtained` (Certificates) each render, computed fresh from the `items` prop right where it's handed to `ListEditor`. **The underlying array/storage order is untouched** — `ListEditor` tracks items by array _index_ for edit/delete targeting (per this doc's Data/API notes above, there's no `id` field), and since every save is a full-array replace anyway, sending that array back in sorted-for-display order is exactly as correct as sending it in whatever order it was in — order was never meaningful to the backend here. Languages/Skills/Contacts are unaffected — no date field, no sorting.
...
7. **Mobile-only: Education/Experience's optional "End Date" field label drops its "(leave empty if ongoing)" explanation, showing just "End Date."** `shared/components/DatePicker.tsx`'s `label` prop accepts a `ReactNode` for exactly this: `ProfileArrayTabs.tsx`'s `END_DATE_LABEL` constant renders two `<span>`s, one `md:hidden` and one `hidden md:inline`, rather than swapping strings in JS. Needed once field labels started forcing `whitespace-nowrap` (see `THEME.md`) — the fuller explanatory text would otherwise overflow a narrow mobile field instead of wrapping. Desktop keeps the full text unchanged.
...
| Contacts     | Network icon (`react-icons`, per the Network Options table below) + Profile Name | `network` (dropdown, see Network Options), `profileName`, `fullUrl`                                                               |
...
| Phone Number                                    | No        | Any international format — validate client-side with `libphonenumber-js` (`isValidPhoneNumber`), matching the backend's use of the C# `PhoneNumbers` library (both are ports of Google's libphonenumber, so validation logic stays consistent between frontend and backend). `type="tel" inputMode="tel"` on the input — on mobile this brings up the phone dial-pad keyboard instead of the full alphanumeric one; doesn't affect desktop or the value itself (still free text, not masked/restricted to digits). |
...
Array-item field validation (Education/Experience/Certificates/Languages/Skills/Contacts individual fields) — see the Array-Item Validation Rules section below.
...
| Field | Required | Rules |
|---|---|---|
| Network | Yes | **Dropdown (curated list), not free text** — see Network Options table below. Structurally eliminates the typo risk the previous free-text approach only "accepted knowingly." |
| Profile Name | Yes | Max 30 chars |
| Full URL | Yes | Must be a valid URL format |

**Network Options — canonical list (source of truth for both this dropdown and `cv.md`'s icon rendering):**

| Value stored in `network` | Icon (`react-icons`)                                                                       |
| ------------------------- | ------------------------------------------------------------------------------------------ |
| GitHub                    | `FaGithub` (`react-icons/fa6`)                                                             |
| LinkedIn                  | `FaLinkedin` (`react-icons/fa6`)                                                           |
| Facebook                  | `FaFacebook` (`react-icons/fa6`)                                                           |
| Gmail                     | `SiGmail` (`react-icons/si`)                                                               |
| Discord                   | `FaDiscord` (`react-icons/fa6`)                                                            |
| X                         | `FaXTwitter` (`react-icons/fa6`)                                                           |
| Telegram                  | `FaTelegram` (`react-icons/fa6`)                                                           |
| Instagram                 | `FaInstagram` (`react-icons/fa6`)                                                          |
| YouTube                   | `FaYoutube` (`react-icons/fa6`)                                                            |
| Stack Overflow            | `FaStackOverflow` (`react-icons/fa6`)                                                      |
| Reddit                    | `FaReddit` (`react-icons/fa6`)                                                             |
| Website                   | `FaGlobe` (`react-icons/fa6`) — generic personal site/portfolio link, not a specific brand |

Twelve options — deliberately curated, not "every platform `react-icons` supports" (that's several thousand entries via the Simple Icons set, far more than a personal contact list ever needs). Extending this list later is a small code change (new dropdown entry + icon import), not something editable at runtime — acceptable tradeoff for a field that changes rarely.

**Supersedes the previous approach** (local `{network}.png` files in `public/icons/networks/`, matched via string lookup with an `onError` fallback — see `cv.md`'s Contacts section for that implementation's history). `react-icons` are SVG components, not static image files: no icon files to source/upload, automatic accent-color theming via `currentColor` on hover (impossible with a flat PNG), and a real fallback (`FaLink`) rather than a broken-image `onError` handler. Backend `ContactDto.network` stays a plain `string` — this constraint is frontend-only (dropdown), matching the `SkillDto.type` string convention already established, not a new backend enum.

### Duplicate prevention

Every array tab also blocks saving an item that's an exact duplicate of another item already in the same list (comparing against every _other_ item — the one currently being edited is excluded from its own comparison, so re-saving a genuinely-unchanged item isn't mistaken for colliding with itself). Comparison is case/whitespace-insensitive (`sameText` in `ProfileArrayTabs.tsx` — trims and lowercases both sides) for free-text fields, exact for dates. Per-tab duplicate key, matching the Owner's own stated criteria (not just "every field matches"):
