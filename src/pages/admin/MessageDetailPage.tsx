...
function DetailField({
  ...
  return (
    ...
    >
      ...
      <div className={`text-text-primary ${valueClassName}`}>{children}</div>
    </div>
  );
}

// IP/Device aren't needed at a glance — a small icon that reveals the value
// in a hover tooltip instead of its own always-visible box.
function IconMeta({
  icon: Icon,
  label,
  value,
  ...
  return (
    <div className="group relative flex items-center justify-center rounded-full border border-border-default p-3 text-text-secondary transition-colors duration-300 hover:text-accent">
      <Icon className="h-5 w-5" />
      <span className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-2 -translate-x-1/2 rounded-lg border border-border-default bg-bg-surface/95 px-3 py-1.5 text-xs whitespace-nowrap text-text-primary opacity-0 shadow-xl backdrop-blur-md transition-opacity duration-300 group-hover:opacity-100">
        {label}: {value}
      </span>
    </div>
  );
